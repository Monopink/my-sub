...
pub fn convert_ruleset(content: &str, ruleset_type: RulesetType) -> String {
    ...
    if let Ok(yaml) = serde_yaml::from_str::<YamlValue>(content) {
        if let Some(payload) = yaml.get("payload").and_then(|value| value.as_sequence()) {
            let mut rule_items_formatted = String::new();
            for item in payload {
                if let Some(rule) = item.as_str() {
                    let rule = rule.trim();
                    if !rule.is_empty() {
                        rule_items_formatted.push_str(rule);
                        rule_items_formatted.push('\n');
                    }
                }
            }

            // If target is Clash Classical, return the payload entries directly.
            if ruleset_type == RulesetType::ClashClassical {
                return rule_items_formatted;
            }

            // Process each payload rule and convert to appropriate format
