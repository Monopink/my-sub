...
use crate::resources::{load_text, resource_exists, ResourceLoadOptions};
...
fn should_forward_request_header(name: &str) -> bool {
    ...
    !matches!(
        lower.as_str(),
        // Compression negotiation is handled by the HTTP client itself.
        "accept-encoding"
            ...
            | "trailer"
            | "upgrade"
            // Transport/framing headers should be generated by client.
            | "host"
            | "content-length"
            | "content-encoding"
    )
...
#[cfg(test)]
mod tests {
    use super::should_forward_request_header;

    #[test]
    fn drops_hop_by_hop_and_encoding_headers() {
    ...
    #[test]
    fn keeps_application_headers() {
        assert!(should_forward_request_header("Authorization"));
        assert!(should_forward_request_header("X-Auth-Token"));
...
impl RuleBases {
    ...
    pub async fn load_content(&self) -> HashMap<SubconverterTarget, String> {
        ...
        // Helper function to load content from any supported resource location
        let load_content = async move |path: &str| -> Option<String> {
            ...
            match load_text(path, &options).await {
                Ok(content) if content.is_empty() => {
                    debug!("Empty content from resource: {}", path);
                    None
                }
                Ok(content) => Some(content),
                Err(e) => {
                    warn!("Failed to load rule base from {}: {}", path, e);
                    None
                }
            }
        };
    ...
    async fn check_external_base(path: &str, dest: &mut String, base_path: &str) -> bool {
        if resource_exists(path, Some(base_path)).await {
            *dest = path.to_string();
            true
        } else {
            false
